...
def SplitTrials(trialindex, trainFraction=0.8, validFraction=0.1):
    ...
    if abs(trainFraction-round(trainFraction,2))>0:
        ...
        validsetsize = int(len(trialindex) * round(validFraction,2))
        shuffle = np.random.permutation(trialindex)
        testIndexes = shuffle[trainsetsize+validsetsize:]
        trainIndexes = shuffle[:trainsetsize]
        validIndexes = shuffle[trainsetsize:trainsetsize+validsetsize]
        return (trainIndexes, validIndexes, testIndexes)
    ...
    for shuffle in Shuffles: # Creating shuffles starting from 1
        for trainFraction in TrainingFraction:
            ...
            if len(trainIndexes)>0:
                ...
                ################################################################################
                auxiliaryfunctions.SaveMetadata(os.path.join(project_path,metadatafilename),data, trainIndexes, validIndexes, testIndexes, trainFraction)
                ################################################################################
...
def create_training_dataset(config,num_shuffles=1,Shuffles=None,windows2linux=False, chatbands=False):
    """
    Creates a training dataset. Labels from all the extracted frames are merged into a single .h5 file.\n
    Only the videos included in the config file are used to create this dataset.\n
    [OPTIONAL]Use the function 'add_new_video' at any stage of the project to add more videos to the project.
    ...
    bodyparts = cfg['bodyparts']
    TrainingFraction = cfg['TrainingFraction']
    ValidFraction = cfg['ValidationFraction']
    for shuffle in Shuffles: # Creating shuffles starting from 1
        for trainFraction, validFraction in zip(TrainingFraction, ValidFraction):
            ...
            if len(trainIndexes)>0:
                ...
                items2change["chAT"] = True
                items2change["optimizer"] = str("adam")
                items2change["adam_lr"] = 0.0001
                items2change["early_stopping_threshold"] = 100
                items2change["display_iters"] = 500
                items2change["save_iters"] = 1000
                items2change["global_scale"] = 1.0
                defaultconfigfile = str(Path(deeplabcut.__file__).parents[0] / 'pose_cfg.yaml')
