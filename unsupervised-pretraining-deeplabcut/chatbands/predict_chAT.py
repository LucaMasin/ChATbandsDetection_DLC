...
def analyze_videos(config,videos=None,videotype='avi',shuffle=1,trainingsetindex=0,gputouse=None,save_as_csv=False, destfolder=None):
    """
    ...
    ##################################################
    if videos is None:
        videos = [os.path.join(cfg['project_path'], 'subimages')]
...
def get_slice_results(tif_path, data_path, dest_path, step_size=20, create_video=True, create_tif=True):
    ...
    try:
        ...
        res_dir = os.path.join(dest_path, tif.split('.')[0])
        os.makedirs(res_dir)
        res = pd.DataFrame(res_list, columns=['Slice', 'X', 'Y_ON', 'P_ON', 'Y_OFF', 'P_OFF'])
        res.to_csv(os.path.join(res_dir, 'result_{}.csv'.format(tif.split('.')[0])))
        if create_video:
            tifstack_2_avi.data_2_mp4(video, os.path.join(res_dir, 'resultvideo_{}'.format(tif.split('.')[0])), 60)
        if create_tif:
            imageio.mimwrite(os.path.join(res_dir, 'resulttif_{}.tif'.format(tif.split('.')[0])), np.uint8(video))
    except Exception as e:
        print(f"Unexpected error for file {tif_path}. Video was not processed due to following error: {e}")
